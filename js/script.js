...
// Falls back to the synced Drive URL once the local base64 copy has been cleared.
function getPlacePhotoSrc(place){
  return place?.photo || place?.photoUrl || '';
}
...
async function resizeImageDataURL(dataUrl, options = {}){
  const {
    ...
    sizeLimitBytes = 280 * 1024,
    qualitySteps = [0.78, 0.7, 0.62, 0.54, 0.46]
  } = options;
...
async function setPhotoFromFile(place, file){
  ...
  try {
    status(`Compressing photo ${place.name || 'Place'}`);
    ...
    renderPassports();
    saveBoard(false);
    status(`Photo added ${humanFileSize(estimateDataUrlBytes(resized))}`);
  } catch (err) {
    console.error(err);
...
async function downloadPassportImage(id){
  ...
  ctx.fillText(place.evaluator || '-', pad + 26, sectionTop + 72);
  ctx.fillText(place.familiarity || '-', pad + 420, sectionTop + 72);
  ...
  ctx.fillText(`${labelForKey(bestKey)} - ${bestVal}/10`, pad + 22, footerY + 68);
  ...
  ctx.font = '600 24px "Avenir Next", Inter, Arial, sans-serif';
  ctx.fillText(`${labelForKey(worstKey)} - ${worstVal}/10`, rightX + 22, footerY + 68);
  ...
  a.click();
  a.remove();
  status(`Image downloaded ${place.name || 'Place'}`);
}
...
function bestBy(key){
  ...
  return best ? `${best.name} (${best.scores[key]}/10)` : '-';
}

function lowestBy(key){
  const weak = [...places].sort((a,b)=>a.scores[key]-b.scores[key])[0];
  return weak ? `${weak.name} (${weak.scores[key]}/10)` : '-';
}
...
const I18N = {
  nl: {
    ...
    sortBy:'Sorteer op', manual:'Aanmaakvolgorde', avgDesc:'Gemiddelde score v', avgAsc:'Gemiddelde score ^', nameAZ:'Naam A-Z', safetyDesc:'Veiligheid v', comfortDesc:'Comfort v', greenDesc:'Groen v', activityDesc:'Activiteit v', inclusionDesc:'Inclusie v', vibeDesc:'Sfeer v', passportHint:'Elke plek wordt een sterker passport met moodlabel, samenvattende zin, tags en duidelijkere vergelijking.',
    notSaved:'Nog niet opgeslagen', csvExported:'CSV geexporteerd {count} plek(ken)', jsonExported:'JSON geexporteerd {count} plek(ken)', imageDownloaded:'Afbeelding gedownload {name}', shared:'Passport gedeeld {name}', copied:'Samenvatting gekopieerd {name}', copiedMsg:'Passport-samenvatting gekopieerd naar klembord.', saved:'Opgeslagen in deze browser {count} plek(ken)', loaded:'Browsersave geladen {count} plek(ken)', imported:'JSON geimporteerd {count} plek(ken)', merged:'{files} JSON-bestand(en) samengevoegd {count} plek(ken)', importInvalid:'Dit JSON-bestand bevat geen geldige place-data.', importInvalidMulti:'Geen geldige plekken gevonden in de geselecteerde JSON-bestanden.', importFailed:'Kon deze JSON-bestanden niet importeren.', storageFull:'Browseropslag is vol', storageFullMsg:'Deze browser raakt vol. Gebruik minder of kleinere foto\'s, vervang grote foto\'s, of exporteer je werk als JSON.', saveFailed:'Kon niet opslaan',
  ...
  en: {
    ...
    sortBy:'Sort by', manual:'Creation order', avgDesc:'Average score', avgAsc:'Average score', nameAZ:'Name', safetyDesc:'Safety', comfortDesc:'Comfort', greenDesc:'Green', activityDesc:'Activity', inclusionDesc:'Inclusion', vibeDesc:'Vibe', passportHint:'Each place becomes a stronger passport with a mood label, summary sentence, tags, and clearer comparison.',
    notSaved:'Not saved yet', csvExported:'CSV exported {count} place(s)', jsonExported:'JSON exported {count} place(s)', imageDownloaded:'Image downloaded {name}', shared:'Passport shared {name}', copied:'Summary copied {name}', copiedMsg:'Passport summary copied to clipboard.', saved:'Saved in this browser {count} place(s)', loaded:'Loaded browser save {count} place(s)', imported:'Imported JSON {count} place(s)', merged:'Merged {files} JSON file(s) {count} place(s)', importInvalid:'This JSON file does not contain valid place data.', importInvalidMulti:'No valid places were found in the selected JSON files.', importFailed:'Could not import these JSON files.', storageFull:'Browser storage is full', storageFullMsg:'This browser is running out of storage. Use fewer or smaller photos, replace large photos, or export your work as JSON.', saveFailed:'Could not save',
...
function addPlace(){ const letter=String.fromCharCode(65+places.length); const p={id:crypto.randomUUID(),name:`${t('placeWord')} ${letter}`,evaluator:'',location:'',placeType:'other',familiarity:'often',note:'',photo:'',photoUrl:'',tags:[],scores:{safety:5,reachability:5,comfort:5,green:5,activity:5,inclusion:5,vibe:5}}; places.push(p); activePlaceId=p.id; render(); saveBoard(true); }
